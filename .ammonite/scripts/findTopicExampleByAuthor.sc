...
import $file.Git, Git.BlameFields, Git.git, Git.attachBlameInformation, Git.attachBlameToMultipleFiles
import $file.^.scripts.files, files.NumberedLine, files.NumberedFileContent
import $file.^.libraries.text, text.stdSpacing
import $file.^.libraries.fileOps, fileOps.FileOperations
...
def fullBlameContent(implicit wd: Path): (String) => Stream[Stream[Git.BlameFields]] = 
  (fileOperations.fullContent _)
    .andThen(attachBlameToMultipleFiles)

def fullSearch: String => Stream[Stream[Git.BlameFields]] = 
...
def unassociatedContent(nfcs: Seq[NumberedFileContent]): Seq[String] = 
  nfcs
    .flatMap(_.content.map{ x=>stdSpacing(x.line)})
    .filter(!_.isEmpty)
